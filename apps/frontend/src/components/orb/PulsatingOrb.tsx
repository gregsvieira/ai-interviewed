...
// Função utilitária para converter HEX para Vector3 do shader
...
export function PulsatingOrb({
  ...
  const frag = /* glsl */ `
    ...
    vec4 draw(vec2 uv) {
      ...
      float len = length(uv);
      // VELOCIDADE REDUZIDA: iTime * 0.2 para ondulação mais lenta
      float n0 = snoise3(vec3(uv * 0.65, iTime * 0.2)) * 0.5 + 0.5;
      float r0 = mix(0.7, 0.9, n0);
      float d0 = distance(uv, (r0 / (len + 0.01)) * uv);
      float v0 = 1.0 / (1.0 + d0 * 10.0);
      v0 *= smoothstep(r0 * 1.05, r0, len);
      // VELOCIDADE REDUZIDA: iTime * 0.8 para transição de cores suave
      float cl = cos(atan(uv.y, uv.x) + iTime * 0.8) * 0.5 + 0.5;
      vec3 colBase = mix(color1, color2, cl);
      vec3 finalCol = mix(backgroundColor, mix(color3, colBase, v0), v0);
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.rot.value += 0.0005;
